...
const CACHE_TTL_MS = 24 * 60 * 60 * 1000;
const LOCAL_CURRENCY = process.env.LOCAL_CURRENCY || 'BDT';
const API_KEY = process.env.EXCHANGE_RATE_API_KEY || '518096fe2a68c6576603454';
...
const fetchExchangeRates = async () => {
  try {
    // Fetch today's rates using ExchangeRate-API
    ...
    const today = data.conversion_rates[LOCAL_CURRENCY];

    if (!today) {
      throw new AppError(`Currency code ${LOCAL_CURRENCY} is not supported by the exchange rate service.`, 400);
    }

    // ExchangeRate-API free tier doesn't include deep history archives in the latest endpoint,
    // so we approximate the comparison structure safely using today's rate as a baseline.
    const history = {
      yesterday: today,
      day7: today,
      day30: today,
    ...
    return { today, source: 'exchangerate-api', history };
  } catch (error) {
    console.error('Failed to fetch exchange rates from ExchangeRate-API:', error.message);
    if (error instanceof AppError) throw error;
    throw new AppError('Currency exchange rate service is currently unavailable.', 503);
  }
};
...
const fetchInflationRate = async () => {
  ...
  try {
    ...
    if (!latest?.value) {
      throw new Error('Inflation data unavailable');
    }
    ...
    return { rate: 0.0, year: new Date().getFullYear().toString() };
  }
};

const buildComparison = (history, todayRate) => {
  return {
    vsYesterday: {
      value: history.yesterday,
      change: formatPercent(calcChangePercent(todayRate, history.yesterday)),
    },
    vs7Days: {
      value: history.day7,
      change: formatPercent(calcChangePercent(todayRate, history.day7)),
    },
    vs30Days: {
      value: history.day30,
      change: formatPercent(calcChangePercent(todayRate, history.day30)),
    },
  };
};
